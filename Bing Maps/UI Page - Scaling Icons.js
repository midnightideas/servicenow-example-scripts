/**
 * UI Page Client Script (Alternative version - Scaling Icons in Bing Maps)
 *
 * UI Page Name: bing_map_page_primary
 *
 * Bing Maps does not scale icon like Google Maps. Therefore, you must specify the exact icon size in the
 * Map Pages otherwise the icon might appear cut-off or off-center. This is an alternative version of
 * the original script, which send the icon image to an external (well, Google, see line 153) image scaling
 * service before displaying. There is one limitation, the external proxy MUST have direct acecss to the icon
...
(function() {
	...
	GwtMapMarker.prototype._draw = function() {
		...
		var pushpinOptions = {
			icon: "https://images1-focus-opensocial.googleusercontent.com/gadgets/proxy?url=" + encodeURIComponent(this.icon) + "&container=focus&resize_w=" + this.iconWidth + "&resize_h=" + this.iconHeight,
